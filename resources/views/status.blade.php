...
@section('content')
    <main class="status py-5 px-5">
        <div class="text-center">
            <h1>Status</h1>
            <p>Please check this page periodically</p>
            <table class="table table-hover mt-3 table-responsive-sm">
                <thead>
                    <tr>
                        @if($userRole == 'USER')
                        ...
                        @foreach($transactions as $trans)
                            @if($trans->jobs->users_id == $userId)
                                <tr>
                                    ...
                                    <td class="align-middle">{{$trans->jobs->role}}</td>
                                    <td class="align-middle">{{$trans->status}}</td>
                                    <td class="align-middle"><a href="http://127.0.0.1:8000/profile/{{$trans->users_id}}">View Profile</a></td>
